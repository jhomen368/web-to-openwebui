...
def test_scrape_command_with_retention_and_upload(mock_upload, mock_crawler_cls, mock_output_mgr_cls, mock_retention_mgr_cls, runner, mock_app_config):
...
@patch('webowui.cli._upload_scrape')
def test_upload_command_from_timestamp(mock_upload_scrape, mock_tracker_cls, runner, mock_app_config):
...
def test_health_command_healthy(mock_asyncio_run, mock_access, mock_client_cls, runner, mock_app_config):
...
@patch('os.access')
@patch('webowui.cli.asyncio.run')
def test_health_command_api_failure(mock_asyncio_run, mock_access, mock_client_cls, runner, mock_app_config):
...
def test_rebuild_state_command(mock_state_mgr_cls, mock_client_cls, mock_cdm_cls, runner, mock_app_config):
    """Test 'rebuild-state' command."""
    mock_app_config.load_site_config.return_value = MagicMock(display_name="Site 1", knowledge_id="kb1")
    mock_app_config.validate_openwebui_config.return_value = []
    mock_app_config.openwebui_api_key = "key"
    ...
    mock_state_mgr.rebuild_from_remote = AsyncMock(return_value=(True, {
        "knowledge_id": "kb1",
        "files_uploaded": 10,
...
@patch('webowui.cli.OpenWebUIClient')
@patch('webowui.cli.StateManager')
def test_rebuild_state_command_real_async(mock_state_mgr_cls, mock_client_cls, mock_cdm_cls, runner, mock_app_config):
...
def test_sync_command(mock_state_mgr_cls, mock_client_cls, mock_cdm_cls, runner, mock_app_config):
    ...
    mock_state_mgr = mock_state_mgr_cls.return_value
    mock_state_mgr.sync_state = AsyncMock(return_value={
        "success": True,
        "local_count": 10,
        "remote_count": 10,
        "in_sync_count": 8,
